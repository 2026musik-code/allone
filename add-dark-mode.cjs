...
replacements.forEach(({ regex, replacement }) => {
  // We need to be careful not to double replace if we run it multiple times
  // But since we are running it once, it should be fine.
  // Let's make sure we don't replace if it already has dark:
  content = content.replace(regex, (match, offset, string) => {
    ...
    if (nextChars.includes('dark:')) {
      return match; // Already has a dark variant
    }
    return replacement;
  });
});
...
console.log('App.tsx updated with dark mode classes');
